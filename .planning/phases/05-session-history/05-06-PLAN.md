...
export async function searchLocalMessages(
  ...
  try {
    const results = await db.all<LocalSearchResult>(sql.raw(ftsQuery), ...params);
    return results;
  } catch (error) {
...
```
  ...
  <done>Local search uses FTS5 with bm25 scoring, HighlightedText renders marked text</done>
</task>
...
2. FTS5 usage: `grep "messages_fts" mobile/services/history/search-service.ts` shows FTS5 query
...
<success_criteria>
- Local search uses FTS5 with bm25 scoring
