...
<objective>
Implement desktop visual polish: centered layout, overlay scrollbars, and platform-appropriate styling.

Purpose: Desktop screens are much wider than mobile. Content should be centered with sensible max-width. Scrollbars should be thin overlays (like Slack/Discord). Visual style should be consistent across platforms while respecting OS conventions.

Output: Layout components and styles that make desktop apps feel polished
</objective>
...
<task type="auto">
  <name>Task 2: Create window persistence and add to platform apps</name>
  <files>
...
Update packages/macos/src/App.tsx to use full desktop features:
```typescript
          ...
          >
            <DesktopLayout centered>
              <View style={styles.content}>
                ...
                <Text style={styles.subtext}>Platform initialized - desktop features active</Text>
              </View>
            </DesktopLayout>
          </CollapsibleSidebar>
        </DropZone>
...
Update packages/windows/src/App.tsx similarly:
```typescript
import React, { useEffect, useState } from 'react';
import { View, Text, StyleSheet, SafeAreaView } from 'react-native';
import { initializePlatform } from './src/platform-init';
...
```
  ...
  <verify>
Run `ls packages/app/src/hooks/useWindowPersistence.ts` - file exists.
Check packages/macos/src/App.tsx and packages/windows/src/App.tsx use desktop features.
  </verify>
  <done>Window persistence hook created, platform apps updated with full desktop feature integration</done>
</task>

<task type="auto">
  <name>Task 3: Update exports and create native scrollbar styles</name>
  <files>
...
Create packages/app/src/styles/index.ts if not exists:
```typescript
export * from './desktop';
```
  </action>
  <verify>
Run `ls packages/app/src/hooks/index.ts packages/app/src/index.ts` - files exist.
Check that all exports are properly included.
  </verify>
  <done>All desktop components, hooks, and styles exported from @dialectic/app</done>
</task>
...
- [ ] macOS App.tsx uses MenuBar, DesktopLayout, CollapsibleSidebar, DropZone
- [ ] Windows App.tsx uses SystemTray, DesktopLayout, CollapsibleSidebar, DropZone
...
4. Platform apps integrate all desktop features
5. Unified codebase shared between macOS and Windows
</success_criteria>
