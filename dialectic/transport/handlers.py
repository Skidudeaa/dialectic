    ...
    async def _handle_send_message(self, conn: Connection, payload: dict) -> None:
        ...
        await self.connections.broadcast(conn.room_id, OutboundMessage(
            ...
            payload={
                ...
                # Same reason references_message_id is here: the column is
                # persisted, but a receiver that only learns of it on the next
                # reload sees an untagged message live and a tagged one after
                # refresh. Omitted when empty so "key present" means "tagged".
                **({"metadata": {"tags": message_tags}} if message_tags else {}),
            }
        ))
