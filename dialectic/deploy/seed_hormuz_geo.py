...
from geo_scopes import LIVE_PREDICATE, insert_scope  # noqa: E402
...
async def main(confirmed_by: UUID, dry_run: bool) -> None:
        ...
        async with conn.transaction():
            for kind, label, geometry, provenance in SEEDS:
                exists = await conn.fetchval(
                    f"SELECT 1 FROM geo_scopes g WHERE g.room_id = $1 AND g.label = $2 AND {LIVE_PREDICATE}",
                    ROOM, label,
                )
                if exists:
                ...
                await conn.execute(
                    ...
                    {"scope_id": str(scope_id), "kind": kind, "subject": subject,
                     "authority": "human_confirmed", "seed": "deploy/seed_hormuz_geo.py"},
                )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--confirmed-by", type=UUID, default=AMO)
    ap.add_argument("--dry-run", action="store_true")
    args = ap.parse_args()
    asyncio.run(main(args.confirmed_by, args.dry_run))
