...
describe('stream destination ownership', () => {
  it('clears stream context in the same transaction as a thread change', () => {
    ...
    useAppStore.getState().receiveStream({ id: 'stream', thread_id: thread.id, references_message_id: 'old-parent' }, 'Old branch text')
    useAppStore.getState().setThread({ ...thread, title: 'New title' })
    expect(useAppStore.getState().streamingMessages.stream.content).toBe('Old branch text')
